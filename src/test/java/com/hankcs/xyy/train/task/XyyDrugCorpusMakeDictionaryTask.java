...
import com.hankcs.xyy.train.cache.EcNlpCustomDictionaryCachePO;
...
public class XyyDrugCorpusMakeDictionaryTask {
    ...
    @Test
    public void generateInsertOrUpdateOnDuplicateSql() throws IOException {
        // 只处理原词
        ...
        Set<EcNlpCustomDictionaryCachePO> ecNlpCustomDictionaryCachePOS = Sets.newHashSetWithExpectedSize(10000);
        Set<String> brands = Sets.newHashSet(FileUtils.readLines(new File(brandDictionaryPath), "UTF-8"));
        brands.remove("");
        brands.remove("20240506202050精准标注20240506202050");
        brands.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.brand.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));

        Set<String> corps = Sets.newHashSet(FileUtils.readLines(new File(corpDictionaryPath), "UTF-8"));
        corps.remove("");
        corps.remove("20240506202050精准标注20240506202050");
        corps.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.corp.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));

        Set<String> specs = Sets.newHashSet(FileUtils.readLines(new File(specDictionaryPath), "UTF-8"));
        specs.remove("");
        specs.remove("20240506202050精准标注20240506202050");
        specs.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.spec.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));

        Set<String> dosages = Sets.newHashSet(FileUtils.readLines(new File(dosageDictionaryPath), "UTF-8"));
        dosages.remove("");
        dosages.remove("20240506202050精准标注20240506202050");
        dosages.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.dosage.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));

        Set<String> cores = Sets.newHashSet(FileUtils.readLines(new File(coreDictionaryPath), "UTF-8"));
        cores.remove("");
        cores.remove("20240506202050精准标注20240506202050");
        cores.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.core.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));

        Set<String> others = Sets.newHashSet(FileUtils.readLines(new File(otherDictionaryPath), "UTF-8"));
        others.remove("");
        others.remove("20240506202050精准标注20240506202050");
        others.remove("20240506202050专有名词20240506202050");
        ecNlpCustomDictionaryCachePOS.addAll(brands.stream()
                .map(word -> EcNlpCustomDictionaryCachePO.builder()
                        .natureName(EcNlpNatureEnum.other.getNature().toString())
                        .word(word).frequency(1024L).build()).collect(Collectors.toSet()));


        // 分组转，一批500个，一个文件1000条批量插入sql
        String insertSqlFilePath = "data/xyy/dictionary/insertsql/insertsql_";
        List<List<List<EcNlpCustomDictionaryCachePO>>> partitionsList = Lists.partition(Lists.partition(Lists.newArrayList(ecNlpCustomDictionaryCachePOS), 500), 1000);
        String formatRowInsertSql = "(''{0}'', ''{1}'', {2}, ''2024-07-05 00:00:00'', ''2024-07-05 00:00:00'')";
        int i = 0;
        for (List<List<EcNlpCustomDictionaryCachePO>> partitions : partitionsList) {
            List<String> batchInsertSqlList = Lists.newArrayListWithExpectedSize(partitions.size());
            for (List<EcNlpCustomDictionaryCachePO> partition : partitions) {
                StringBuilder batchInsertSql = new StringBuilder("INSERT INTO xyy_ec_search.`tb_ec_nlp_custom_dictionary` (`nature_name`,`word`,`frequency`,`create_time`,`update_time`) VALUES ");
                List<String> rowInsertSqls = Lists.newArrayListWithExpectedSize(partition.size());
                partition.forEach(item -> rowInsertSqls.add(MessageFormat.format(formatRowInsertSql, item.getNatureName(), item.getWord(), String.valueOf(item.getFrequency()))));
                batchInsertSql.append(String.join(",", rowInsertSqls)).append(";");
                batchInsertSqlList.add(batchInsertSql.toString());
            }
            i++;
            // 写入文件
            File insertSqlFile = new File(insertSqlFilePath + i + ".sql");
            FileUtils.writeLines(insertSqlFile, batchInsertSqlList);
        }
    }
